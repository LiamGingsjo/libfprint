...
static gboolean
synatlsmoc_is_provisioned (FpiDeviceSynaTlsMoc *self)
{
...
static gboolean
synatlsmoc_key_flag (FpiDeviceSynaTlsMoc *self)
{
  return (self->iface & KEY_FLAG_MASK) != 0;
}

static gboolean
load_uncompressed_public_key (guint8 *key, gsize key_len, EVP_PKEY **pkey, GError **error)
{
...
static gboolean
sensor_pub_key_compatibility_check (
...
static void
synatlsmoc_load_sensor_key (FpiDeviceSynaTlsMoc *self)
{
...
static gboolean
sensor_certificate_from_raw (Certificate *self, guint8 *data, gsize len, GError **error)
{
  GError *local_error = NULL;
...
static gboolean
parse_certificates_within_self (FpiDeviceSynaTlsMoc *self,
                                GError **error)
{
  GError *local_error = NULL;
...
static gboolean
status_is_success (guint16 status)
{
...
static gboolean
sensor_status_is_result_bad_param (guint16 status)
{
...
static void
synatlsmoc_cmd_receive_cb (FpiUsbTransfer *transfer,
                           FpDevice *device,
                           gpointer userdata,
                           GError *error)
{
  ...
  if (callback)
    callback (device, unwrapped, unwrapped_len, NULL);
}
...
static void
synatlsmoc_cmd_ssm_done (FpiSsm *ssm, FpDevice *device, GError *error)
{
...
static gboolean
fpi_byte_reader_get_mis_version (FpiByteReader *reader,
                                 MisVersion *result)
{
  gboolean read_ok = TRUE;
...
static void
send_get_version (FpiDeviceSynaTlsMoc *self)
{
...
static void
recv_get_version_tls_force_close (FpDevice *device,
                                  guchar *buffer_in,
                                  gsize length_in,
                                  GError *error)
{
...
static void
send_cmd_to_force_close_sensor_tls_session (
...
static void
send_tls_data (FpiDeviceSynaTlsMoc *self, guint8 *tdata, gsize tdata_size)
{
 ...
 */
static void
send_pair (FpiDeviceSynaTlsMoc *self,
...
static void
fp_dbg_enroll_stats (EnrollStats *enroll_stats)
{
  fp_dbg ("Enroll stats:");
...
static gboolean
fpi_byte_reader_get_db2_info (FpiByteReader *reader,
                              Db2Info *db2_info)
{
  gboolean read_ok = TRUE;
...
static void
fp_dbg_db2_info (Db2Info *db2_info)
{
...
static void
send_db2_info (FpiDeviceSynaTlsMoc *self)
{
...
static gsize
get_object_list_recv_size (FpiDeviceSynaTlsMoc *self,
                           ObjType obj_type)
{
  ListData *data = fpi_ssm_get_data (self->task_ssm);
  ...
  switch (obj_type)
    {
    case OBJ_TYPE_USERS:
      return 4 + 16 * data->num_current_users;
      break;
    case OBJ_TYPE_TEMPLATES:
      return 4 + 16 * data->num_current_templates;
      break;
    case OBJ_TYPE_PAYLOADS:
      return 4 + 16 * data->num_current_payloads;
      break;
    default:
                          ...
                          const Db2Id obj_id)
{
  ...
  switch (obj_type)
    {
    case OBJ_TYPE_PAYLOADS:
      callback = recv_db2_get_payload_list;
      break;
    case OBJ_TYPE_TEMPLATES:
      callback = recv_db2_get_template_list;
      break;
    case OBJ_TYPE_USERS:
    default:
      g_assert_not_reached ();
...
recv_db2_get_template_info (FpDevice *device, guint8 *buffer_in, gsize length_in, GError *error)
{
  ...
  if (read_ok)
    memcpy (data->user_id, user_id_offset, DB2_ID_SIZE);
  read_ok &= fpi_byte_reader_dup_data (&reader, 16, &info_18_33);
  read_ok &= fpi_byte_reader_dup_data (&reader, 12, &info_34_45);
                          ...
                          const Db2Id obj_id)
{
  ...
  switch (obj_type)
    {
    case OBJ_TYPE_PAYLOADS:
      callback = recv_db2_get_payload_info;
      break;
    case OBJ_TYPE_TEMPLATES:
      callback = recv_db2_get_template_info;
      break;
    case OBJ_TYPE_USERS:
    default:
      g_assert_not_reached ();
                          ...
                          gsize obj_data_size)
{
  ...
  switch (obj_type)
    {
    case OBJ_TYPE_PAYLOADS:
      callback = recv_db2_get_payload_data;
      break;
    case OBJ_TYPE_TEMPLATES:
    case OBJ_TYPE_USERS:
    default:
...
static void
recv_db2_cleanup (FpDevice *device, guchar *buffer_in, gsize length_in, GError *error)
{
  FpiDeviceSynaTlsMoc *self = FPI_DEVICE_SYNATLSMOC (device);
  if (error != NULL)
    {
...
static void
send_db2_cleanup (FpiDeviceSynaTlsMoc *self)
{
  const guint send_size = 2;
...
static void
synatlsmoc_tls_status_cb (FpiUsbTransfer *transfer, FpDevice *device, gpointer userdata, GError *error)
{
  FpiDeviceSynaTlsMoc *self = FPI_DEVICE_SYNATLSMOC (device);
...
static void
synatlsmoc_get_tls_status (FpiDeviceSynaTlsMoc *self, FpiSsm *ssm)
{
  FpDevice *device = FP_DEVICE (self);
...
static char *
export_private_key_to_pem (EVP_PKEY *pkey, GError **error)
{
...
synatlsmoc_open_run_state (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case OPEN_EXIT_BOOTLOADER:
      ...
      break;
    case OPEN_LOAD_SENSOR_KEY:
      synatlsmoc_load_sensor_key (self);
      break;
...
static void
synatlsmoc_cancel (FpDevice *device)
{
...
static void
synatlsmoc_suspend (FpDevice *device)
{
...
static void
close_tls_session (FpiDeviceSynaTlsMoc *self)
{
  GError *error = NULL;
  g_autofree guint8 *tls_data = NULL;
...
static void
synatlsmoc_close (FpDevice *device)
{
...
static void
synatlsmoc_list_run_state (FpiSsm *ssm, FpDevice *device)
{
  FpiDeviceSynaTlsMoc *self = FPI_DEVICE_SYNATLSMOC (device);
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case LIST_DB2_GET_PAYLOAD_LIST:
      {
        if (data->list_payload_id)
          g_ptr_array_free (data->list_payload_id, TRUE);
        data->list_payload_id = g_ptr_array_new_with_free_func (g_free);

        if (data->current_template_id)
          g_free (data->current_template_id);
    ...
    case LIST_DB2_GET_PAYLOAD_INFO:
      {
        if (data->current_payload_id)
          g_free (data->current_payload_id);
...
static void
synatlsmoc_list (FpDevice *device)
{
...
static void
synatlsmoc_delete_ssm_done (FpiSsm *ssm, FpDevice *dev, GError *error)
{
  synatlsmoc_task_ssm_done (ssm, dev, error);
...
static gboolean
get_template_id_from_fp_print (FpPrint *print, Db2Id template_id, GError **error)
{
  gboolean ret = TRUE;
...
static void
synatlsmoc_delete (FpDevice *device)
{
...
static void
sensor_add_enrollment (FpiDeviceSynaTlsMoc *self, Db2Id template_id, FpUserId fp_user_id, FingerId finger_id)
{
...
synatlsmoc_enroll_run_state (FpiSsm *ssm, FpDevice *device)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case ENROLL_SET_EVENT_NONE:
      ...
      send_event_config (self, NO_EVENTS);
      break;
    case ENROLL_SEND_FRAME_FINISH:
      sensor_frame_finish (self);
      break;
    case ENROLL_SEND_ADD_IMAGE:
      send_add_image (self);
      break;
...
synatlsmoc_identify_verify_run_state (FpiSsm *ssm, FpDevice *device)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case IDENTIFY_VERIFY_SET_EVENT_NONE:
      ...
      send_event_config (self, NO_EVENTS);
      break;
    case IDENTIFY_VERIFY_SEND_FRAME_FINISH:
      sensor_frame_finish (self);
      break;
...
static void
synatlsmoc_identify_verify (FpDevice *device)
{
...
static void
synatlsmoc_clear_storage_run_state (FpiSsm *ssm, FpDevice *device)
{
  FpiDeviceSynaTlsMoc *self = FPI_DEVICE_SYNATLSMOC (device);

  switch (fpi_ssm_get_cur_state (ssm))
    {
    case CLEAR_STORAGE_DB2_FORMAT:
...
static void
synatlsmoc_clear_storage (FpDevice *device)
{
...
static void
fpi_device_synatlsmoc_init (FpiDeviceSynaTlsMoc *self)
{
  G_DEBUG_HERE ();
}
